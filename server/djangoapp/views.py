...
def login_user(request):
    ...
    if user is not None:
        ...
        login(request, user)
        data = {"userName": username, "status": "Authenticated"} # Update login status of current user to autehenticated on their json data
    return JsonResponse(data) # Then return their updated data?
...
# Create a `registration` view (Implementation) to handle sign up request, where the function, when the user registers, creates a user object, and loggs them in. It should return a JSON object with the username.
@csrf_exempt
